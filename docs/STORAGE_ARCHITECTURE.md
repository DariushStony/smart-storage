# 🏗️ StorageVault Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                       Your DK Application                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │  Component   │  │  Component   │  │  Component   │          │
│  │      A       │  │      B       │  │      C       │          │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘          │
│         │                 │                 │                   │
│         │  import         │  import         │  import           │
│         │                 │                 │                   │
│         ▼                 ▼                 ▼                   │
│  ┌──────────────────────────────────────────────────┐          │
│  │   apps/dk/src/shared/utils/storage.ts            │          │
│  │                                                   │          │
│  │  • userPrefsVault    ← User settings             │          │
│  │  • cacheVault        ← API responses             │          │
│  │  • sessionVault      ← Temporary state           │          │
│  │  • featureFlagsVault ← A/B tests                 │          │
│  │  • analyticsVault    ← Tracking                  │          │
│  │  • draftVault        ← Form auto-save            │          │
│  │  • growthbookVault   ← Experiments               │          │
│  │  • cartVault         ← Shopping cart             │          │
│  └───────────────────┬──────────────────────────────┘          │
│                      │ uses                                     │
│                      ▼                                          │
│  ┌──────────────────────────────────────────────────┐          │
│  │   libs/utils/src/storage/index.ts                │          │
│  │                                                   │          │
│  │  • defaultStorageVault                           │          │
│  │  • getStorageSlice()                             │          │
│  │  • StorageVault class                            │          │
│  │                                                   │          │
│  │  Core Features:                                  │          │
│  │  • TTL expiration                                │          │
│  │  • Debounced writes                              │          │
│  │  • Automatic cleanup                             │          │
│  │  • SSR fallback                                  │          │
│  │  • Error handling                                │          │
│  └───────────────────┬──────────────────────────────┘          │
│                      │                                          │
│         ┌────────────┴────────────┐                            │
│         │                         │                            │
│         ▼                         ▼                            │
│  ┌──────────────┐         ┌──────────────┐                    │
│  │ localStorage │         │ sessionStorage│                    │
│  │              │         │               │                    │
│  │ (Browser)    │         │ (Browser)     │                    │
│  └──────────────┘         └──────────────┘                    │
│         │                         │                            │
│         │ SSR? Use Memory         │                            │
│         ▼                         ▼                            │
│  ┌──────────────┐         ┌──────────────┐                    │
│  │ Map (Memory) │         │ Map (Memory)  │                    │
│  │              │         │               │                    │
│  │ (Server)     │         │ (Server)      │                    │
│  └──────────────┘         └──────────────┘                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    Initialization Flow                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. App Starts                                                   │
│     └─> apps/dk/src/pages/_app.jsx                              │
│                                                                  │
│  2. Calls initializeApp()                                        │
│     └─> apps/dk/src/shared/services/init-app.ts                 │
│                                                                  │
│  3. Initializes Storage with Sentry Logger                       │
│     └─> initializeStorageLogger({ log: sendLog })               │
│                                                                  │
│  4. ✅ Storage Ready!                                            │
│     └─> All vaults now available throughout app                 │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                      Data Flow Example                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Component:                                                      │
│  ┌────────────────────────────────────────────────────┐         │
│  │ import { userPrefsVault } from '@dk/.../storage';  │         │
│  │                                                     │         │
│  │ // Set theme                                       │         │
│  │ userPrefsVault.setItem('theme', 'dark');          │─────┐   │
│  │                                                     │     │   │
│  │ // Get theme                                       │     │   │
│  │ const theme = userPrefsVault.getItem('theme');    │◄────┤   │
│  └────────────────────────────────────────────────────┘     │   │
│                                                              │   │
│  StorageVault:                                               │   │
│  ┌────────────────────────────────────────────────────┐     │   │
│  │ 1. Validate key                                    │     │   │
│  │ 2. Add to dirtyData (read-after-write)            │     │   │
│  │ 3. Start debounce timer                            │     │   │
│  │ 4. After 100ms → Save to storage                  │     │   │
│  │ 5. Setup pagehide listener (auto-flush)           │     │   │
│  │ 6. On error → Log to Sentry                       │     │   │
│  └────────────────────────────────────────────────────┘     │   │
│                               │                              │   │
│                               ▼                              │   │
│  localStorage:                                               │   │
│  ┌────────────────────────────────────────────────────┐     │   │
│  │ Key: "DK_USER_PREFS"                               │     │   │
│  │ Value: {                                           │     │   │
│  │   "theme": {                                       │     │   │
│  │     "value": "dark",                               │     │   │
│  │     "expiry": null                                 │     │   │
│  │   }                                                │     │   │
│  │ }                                                  │◄────┘   │
│  └────────────────────────────────────────────────────┘         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    Vault Selection Guide                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Critical Data (Orders, Payments)                                │
│  └─> userPrefsVault / cartVault (0ms debounce)                  │
│                                                                  │
│  User Settings (Theme, Language)                                 │
│  └─> userPrefsVault (0ms debounce, permanent)                   │
│                                                                  │
│  API Cache (Products, Categories)                                │
│  └─> cacheVault (200ms debounce, 5-10 min TTL)                  │
│                                                                  │
│  Temporary State (Wizard, Multi-step Forms)                      │
│  └─> sessionVault (sessionStorage, cleared on tab close)        │
│                                                                  │
│  A/B Tests & Experiments                                         │
│  └─> featureFlagsVault / growthbookVault (100ms debounce)       │
│                                                                  │
│  High-Frequency Updates (Analytics, Events)                      │
│  └─> analyticsVault (500ms debounce)                            │
│                                                                  │
│  Form Auto-Save                                                  │
│  └─> draftVault (1000ms debounce, 24h TTL)                      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                     Singleton Pattern                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ComponentA.tsx:                                                 │
│  import { userPrefsVault } from '@dk/.../storage';              │
│  userPrefsVault.setItem('theme', 'dark');                       │
│                    │                                             │
│                    │ Same Instance                               │
│                    ▼                                             │
│  ComponentB.tsx:                                                 │
│  import { userPrefsVault } from '@dk/.../storage';              │
│  const theme = userPrefsVault.getItem('theme'); // 'dark' ✅    │
│                                                                  │
│  ✅ All imports share the same singleton instance               │
│  ✅ Changes in one place visible everywhere                     │
│  ✅ No need to pass down via props/context                      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                   SSR vs Client Behavior                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Server-Side (SSR):                                              │
│  ┌────────────────────────────────────────────────┐             │
│  │ typeof window === 'undefined'                  │             │
│  │                                                │             │
│  │ StorageVault uses:                             │             │
│  │ • Map<string, string> (in-memory)             │             │
│  │ • No persistence                              │             │
│  │ • Per-request isolation                       │             │
│  │ • No pagehide listeners                       │             │
│  └────────────────────────────────────────────────┘             │
│                                                                  │
│  Client-Side (Browser):                                          │
│  ┌────────────────────────────────────────────────┐             │
│  │ typeof window !== 'undefined'                  │             │
│  │                                                │             │
│  │ StorageVault uses:                             │             │
│  │ • localStorage / sessionStorage               │             │
│  │ • Full persistence                            │             │
│  │ • Shared across tabs (localStorage)           │             │
│  │ • Auto-flush on pagehide                      │             │
│  └────────────────────────────────────────────────┘             │
│                                                                  │
│  Hydration Pattern:                                              │
│  ┌────────────────────────────────────────────────┐             │
│  │ 1. Server renders with in-memory data          │             │
│  │ 2. Client loads and creates new instance       │             │
│  │ 3. useEffect hydrates client storage           │             │
│  │                                                │             │
│  │ useEffect(() => {                              │             │
│  │   vault.setItem('data', serverData);           │             │
│  │ }, []);                                        │             │
│  └────────────────────────────────────────────────┘             │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

## Key Takeaways

1. **Centralized Configuration**: All vaults pre-configured in `storage.ts`
2. **Singleton Pattern**: One instance per vault across entire app
3. **Auto-Initialization**: Setup once in `_app.jsx`, use everywhere
4. **SSR Safe**: Automatic fallback to in-memory storage
5. **Error Handling**: All errors logged to Sentry automatically
6. **Type Safe**: Full TypeScript support with generics
7. **Performance**: Debounced writes with read-after-write consistency

## Files Overview

```
apps/dk/
├── src/
│   ├── pages/
│   │   └── _app.jsx                    ← Initialization (done!)
│   └── shared/
│       ├── services/
│       │   ├── init-app.ts             ← Setup storage logger
│       │   └── growthbook/
│       │       └── cache.tsx           ← Example migration
│       └── utils/
│           ├── storage.ts              ← 8 pre-configured vaults ⭐
│           ├── STORAGE_GUIDE.md        ← Complete usage guide
│           └── ...
├── STORAGE_SETUP_COMPLETE.md           ← Setup summary
├── STORAGE_QUICK_REF.md                ← Quick reference
└── MIGRATION_GUIDE.md                  ← Migration patterns

libs/utils/src/storage/
├── index.ts                            ← Core implementation
├── setup.ts                            ← Logger initialization
├── README.md                           ← Full API docs
└── docs/                               ← Additional documentation
```

---

**Everything is ready! Just import and use!** 🚀
